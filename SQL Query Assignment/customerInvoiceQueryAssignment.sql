...
select invoice.inv_number, customer.cus_fname, customer.cus_lname
from invoice
join customer on invoice.cus_code = customer.cus_code
where cus_fname = 'Alfred' and cus_lname = 'Ramas';
...
JOIN INVOICE on INVOICE.inv_number = LINE.inv_number
JOIN CUSTOMER on INVOICE.CUS_CODE = CUSTOMER.CUS_CODE
...
-- 6. Use SELECT DISTINCT to list vendor names and product descriptions (Vendor Name and Product Description),
-- matching products with their vendors using keys. Use a FULL OUTER JOIN so that unmatched vendors and products will
-- still be in the output. (output = 2 columns)
...
-- 7. Use SELECT to create a simple list of invoices (invoide number) belonging to those customers in the '615' area
-- code. You will need to join customer to invoice on two criteria: matching cus_code AND cus_areacode matching the
-- string above. (output = 1 column)
SELECT  inv_number
from INVOICE
JOIN CUSTOMER on INVOICE.CUS_CODE = CUSTOMER.CUS_CODE AND CUSTOMER.CUS_AREACODE = '615';


-- 8. Use aggregate functions and math operations to total up the amount of money spent by every single individual in
-- the database (first/last names, total). You will need to join customer with invoice and line and output first and
-- last names with dollar amounts. (output = 3 columns)
...
FROM CUSTOMER
LEFT OUTER JOIN INVOICE on CUSTOMER.CUS_CODE = INVOICE.CUS_CODE
